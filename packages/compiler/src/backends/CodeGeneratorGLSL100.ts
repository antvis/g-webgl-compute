...
`;
  ...
  public generateExpression(
    ...
    if (isScalar(expression?.type)) {
      ...
      // tslint:disable-next-line:no-console
      console.warn(
        `[GLSL100 compiler]: unknown expression: ${expression}`,
        dataType,
      );
    }
    return '';
  ...
  public generateMemberExpression(expression: MemberExpression): string {
    ...
    if (expression.property.type === AST_NODE_TYPES.Identifier) {
      ...
      }[${this.generateExpression(expression.property)}]`;
    }
  }
  ...
  public generateAssignmentExpression(
    ...
    return `${left} ${expression.operator} ${
      castVec4 ? 'vec4(' : ''
    }${this.generateExpression(expression.right, dataType)}${
      castVec4 ? ')' : ''
    }`;
  }
