...
export class Transformer {
  ...
  public transform(program: Program, context?: GLSLContext): ShaderProgram {
    ...
    programNode.body = flatten(
      program.body.map((stmt) =>
        this.transformStatement(stmt, programNode, programNode),
      ),
    ).filter((stmt) => stmt);
    return programNode;
  }
  ...
  private transformStatement(
    ...
    if (stmt.type === AST_NODE_TYPES.VariableDeclaration) {
      shaderStatement = this.transformVariableDeclaration(
        stmt,
        scopeNode,
        parentNode,
      );
      ...
      shaderStatement = this.transformClassDeclaration(
        stmt,
        scopeNode,
        parentNode,
      );
  ...
  private transformExpression(
    ...
    if (expression) {
      if (expression.type === AST_NODE_TYPES.Identifier) {
        // 尝试在当前 Scope 中查找
        const existedIdentifier = getIdentifierFromScope(
          parentNode,
          expression.name,
        );
        retExpression = {
        ...
        // 右值类型由左侧推导
        const leftRightDataType =
          type || this.inference.infer(expression, parentNode);
        ...
        retExpression.elements = expression.elements.map(
          (e) =>
            this.transformExpression(e, retExpression!, componentDataType)!,
        );
  ...
  private transformIdentifier(
    ...
    return {
      ...
      // TODO: 考虑 Struct
      dataType:
        (typeAnnotation && dataTypeMap[typeAnnotation]) ||
        SHADER_AST_TOKEN_TYPES.Float,
      name,
      parent,
    };
  ...
  private transformDoWhileStatement(
    ...
    // @ts-ignore
    doWhileStatement.test = this.transformExpression(
      stmt.test,
      doWhileStatement,
    );
    // @ts-ignore
    doWhileStatement.body = this.transformStatement(
      stmt.body,
      doWhileStatement,
      doWhileStatement,
    );
  ...
  private transformWhileStatement(
    ...
    // @ts-ignore
    whileStatement.body = this.transformStatement(
      stmt.body,
      whileStatement,
      whileStatement,
    );
    ...
    const shaderStatement: ShaderVariableDeclaration = {
      ...
      declarations: stmt.declarations.map((d) => {
        ...
        const declarator: ShaderVariableDeclarator = {
          ...
          storageClass,
          init:
            d.init && this.transformExpression(d.init, parentNode, id.dataType),
        };
  ...
  private transformFunctionDeclaration(
    ...
    const returnType =
      (stmt.returnType && dataTypeMap[stmt.returnType]) ||
      SHADER_AST_TOKEN_TYPES.Float;
    ...
    shaderStatement.body.body = flatten(
      stmt.body.body.map((b) =>
        this.transformStatement(b, shaderStatement, shaderStatement.body),
      ),
    );
  ...
  private transformIfStatement(
    ...
    shaderStatement.test = this.transformExpression(
      stmt.test,
      shaderStatement,
    )!;
        ...
        classProperties.forEach((property) => {
          if (property.key.type === AST_NODE_TYPES.Identifier) {
            ...
            declaration.declarations.push(
              declarator as ShaderVariableDeclarator,
            );
            // @ts-ignore
            id.parent = declarator;
          }
  ...
  private analyzeDecorators(
    ...
    analyzeDecorators.forEach(({ name, params }) => {
      if (name === PropertyDecorator.Shared) {
        ...
        let existed = this.context.uniforms.find(
