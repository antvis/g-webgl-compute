  ...
  describe('Statement', () => {
    describe('Variable Declaration', () => {
      ...
      test('should transform variable declaration statement correctly.', () => {
        ...
        // 未声明类型，默认使用 f32
        expect((declarator.init as ArrayExpression).type).toBe(
          AST_NODE_TYPES.ArrayExpression,
        );
        expect((declarator.init as ArrayExpression).elements.length).toBe(3);
        expect((declarator.init as ArrayExpression).elements[0].type).toBe(
          AST_TOKEN_TYPES.Float,
        );
        expect(
          ((declarator.init as ArrayExpression).elements[0] as Scalar).value,
        ).toBe(1);
      });
      ...
      test('should transform variable declaration statement with type annotation correctly.', () => {
        ...
        expect((declarator2.init as Literal).type).toBe(
      ...
      test('should generate numthreads statement correctly.', () => {
        ...
        expect(shaderProgram.body.length).toBe(1);
        const numthreadsDeclaration = shaderProgram
          .body[0] as NumThreadStatement;
        expect(numthreadsDeclaration.type).toBe(
          AST_NODE_TYPES.NumThreadStatement,
        );
      ...
      test('should generate variable declaration with storage class correctly.', () => {
        ...
        expect(shaderProgram.body.length).toBe(1);
        const variableDeclaration = shaderProgram
          .body[0] as VariableDeclaration;
        expect(variableDeclaration.type).toBe(
          AST_NODE_TYPES.VariableDeclaration,
        );
        expect(variableDeclaration.declarations.length).toBe(2);
        expect(variableDeclaration.declarations[0].id.name).toBe('p1');
        expect(variableDeclaration.declarations[0].id.dataType).toBe(
          AST_TOKEN_TYPES.Float,
        );
        expect(variableDeclaration.declarations[0].storageClass).toBe(
          STORAGE_CLASS.Uniform,
        );
        expect(variableDeclaration.declarations[1].id.name).toBe('p2');
        expect(variableDeclaration.declarations[1].id.dataType).toBe(
          AST_TOKEN_TYPES.FloatArray,
        );
        expect(variableDeclaration.declarations[1].storageClass).toBe(
    ...
    describe('Function Declaration', () => {
      test('should transform function declaration statement with type annotation correctly.', () => {
        ...
        // const a: int = 1;
        expect(declaration1.body.body[0].type).toBe(
          AST_NODE_TYPES.VariableDeclaration,
        );

        // a = a + 2;
        const expressionStatement = declaration1.body
        ...
        expect(returnStatement.argument?.type).toBe(
        ...
        expect((binaryExpression.left as Identifier).dataType).toBe(
      ...
      test('should use builtin variables in root scope correctly.', () => {
        ...
        expect(shaderProgram.body.length).toBe(1);
        const functionDeclaration = shaderProgram
          .body[0] as FunctionDeclaration;
        expect(functionDeclaration.type).toBe(
          AST_NODE_TYPES.FunctionDeclaration,
        );

        // const a = globalInvocationID;
        const variableDeclaration = functionDeclaration.body
          .body[0] as VariableDeclaration;
        expect(variableDeclaration.type).toBe(
          AST_NODE_TYPES.VariableDeclaration,
        );

        const declarator1 = variableDeclaration
          .declarations[0] as VariableDeclarator;
        expect(declarator1.id.type).toBe(AST_NODE_TYPES.Identifier);
        expect(declarator1.id.name).toBe('a');
        // 这里不进行类型转换，在生成代码时进行
        expect(declarator1.id.dataType).toBe(AST_TOKEN_TYPES.Vector3Int);
        expect((declarator1.init as Identifier).type).toBe(
          AST_NODE_TYPES.Identifier,
        );
        expect((declarator1.init as Identifier).dataType).toBe(
    ...
    describe('Control Flow', () => {
      ...
      test('should transform for statement with AssignmentExpression correctly.', () => {
        ...
        expect((forStatement.test as BinaryExpression).type).toBe(
          AST_NODE_TYPES.BinaryExpression,
        );
        expect((forStatement.test as BinaryExpression).right.type).toBe(
          AST_TOKEN_TYPES.Int32,
        );
        // @ts-ignore
