...
export class Chart {
  ...
  public async init() {
    const rawData = await d3.csv(this.options.data);
    rawData.forEach((d, i) => {
      d.$unitChartId = `${i}`;
    });
    ...
    this.options.layouts.forEach(({ name, layouts: ls }) => {
      ...
      this.containerMap[name] = container.rootContainer;
    });
  ...
  private renderAxis({ layout }: { layout: string }) {
    ...
    this.containerMap[layout].contents.forEach(({ label, visualspace }) => {
